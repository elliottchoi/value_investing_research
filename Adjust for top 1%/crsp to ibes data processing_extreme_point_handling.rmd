...
ibes_pull_us<- inner_join(ibes_pull_us,base_tickers_for_IBES,by = c("oftic"="TICKER"))
...
# Filter out all the cusips we dont need b/c they are not in the crsp database
ibes_pull_int<- inner_join(ibes_pull_int,base_tickers_for_IBES,by = c("oftic"="TICKER"))
...
ibes <- ibes %>% group_by(oftic,year_of_analyst_forecast,month_of_analyst_forecast,forecast_period_end_year) %>% mutate (number_of_analyst_predictions = n_distinct(anndats,analys))
...
ibes <- ibes %>% group_by(oftic,forecast_period_end_year) %>% mutate (number_of_months_per_forecast_period_end = n_distinct(month_and_year_of_analyst_forecast))
ibes
```
...
ibes_t<- ibes_t %>% group_by(oftic,fpedats)%>% mutate(number_of_consecutive_months = n_distinct(month_and_year_of_analyst_forecast))
...
ibes_t_plus_one<- ibes_t_plus_one %>% group_by(oftic,fpedats)%>% mutate(number_of_consecutive_months = n_distinct(month_and_year_of_analyst_forecast))
...
IBES_DATABASE<- IBES_DATABASE %>% group_by(oftic,fpedats)%>% mutate(total_consectutive_months = n_distinct(month_and_year_of_analyst_forecast))
IBES_DATABASE <- IBES_DATABASE %>% filter(total_consectutive_months >=20)
```
Display the Database
```{r}
IBES_DATABASE <- arrange(IBES_DATABASE, oftic, anndats,fpedats)
IBES_DATABASE
```
Save IBES Database
```{r}
...
res <- dbSendQuery(wrds, "select cusip, pends,oftic,value from ibes.act_epsus
...
ibes_actuals_us <- inner_join(ibes_actuals_us,crsp_data_ticker,by =c("oftic"="TICKER"))
...
res <- dbSendQuery(wrds, "select cusip,pends,oftic, value from ibes.act_epsint
                   where pends between '1980-01-01' and '2018-12-31'
                   and pdicity = 'ANN'
                   and cusip !=''")
ibes_actuals_int<- dbFetch(res, n=-1)
dbClearResult(res)
ibes_actuals_int <- inner_join(ibes_actuals_int,crsp_data_ticker,by =c("oftic"="TICKER"))
...
ibes_actuals<-ibes_actuals%>%distinct(oftic,pends,.keep_all = TRUE)
```
Parse the Results
```{r}
...
col_for_IBES_actuals<-c('pends','actual_annual_eps','oftic')
...
IBES_DATABASE<- inner_join(IBES_DATABASE,ibes_actuals, by = c("oftic"="oftic","fpedats"="pends" ))
# Add Analyst Standard Deviation to each other 
IBES_DATABASE<- IBES_DATABASE %>% group_by(oftic,month_and_year_of_analyst_forecast)%>% mutate(forecast_standard_deviation = sd(value))
...
FULL_DATABASE <- inner_join(IBES_DATABASE,crsp_data_v1, by = c('oftic'='TICKER','month_of_analyst_forecast'='month','year_of_analyst_forecast'='year'))
...
FULL_DATABASE <- FULL_DATABASE%>%group_by(oftic, year_of_analyst_forecast,month_of_analyst_forecast,forecast_end_period)%>% mutate(average_forecasted_earnings = mean(f_eps))
...
june_full_database<- june_full_database %>% distinct(oftic,month_of_analyst_forecast,.keep_all=TRUE)
...
distinct_FULL_DATABASE<- FULL_DATABASE_v01 %>% distinct(oftic,year_of_analyst_forecast,month_of_analyst_forecast,.keep_all = TRUE)
# Stats for George 
...
nrow(distinct_FULL_DATABASE%>% distinct(cusip))
nrow(distinct_quartiled_1980_1996%>% distinct(cusip))
nrow(distinct_quartiled_1997_2018%>% distinct(cusip))
```
